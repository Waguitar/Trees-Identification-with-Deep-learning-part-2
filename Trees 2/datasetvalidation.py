...
class DataSetvalidation:
    def __init__(self, in_dir, exts='.jpg'):
        ...
        for name in os.listdir(in_dir):
            ...
            if os.path.isdir(current_dir):
                ...
                filenames_validation = self._get_filenames(os.path.join(current_dir, 'validation'))

                # Append them to the list of all filenames for the test-set.
                self.filenames_validation.extend(filenames_validation)

                # Create an array of class-numbers.
                class_numbers = [class_number] * len(filenames_validation)

                # Append them to the list of all class-numbers for the test-set.
                self.class_numbers_validation.extend(class_numbers)
    ...
    def get_paths(self, validation=False):
        """
        ...
        if validation:
            ...
            filenames = self.filenames_validation
            class_numbers = self.class_numbers_validation
        ...
        for filename, cls in zip(filenames, class_numbers):
            ...
            path = os.path.join(self.in_dir, self.class_names[cls], validation_dir, filename)
    ...
    def get_validation_set(self):
        ...
        return list(self.get_paths(validation=True)), \
               np.asarray(self.class_numbers_validation), \
               one_hot_encoded(class_numbers=self.class_numbers_validation,
                               num_classes=self.num_classes)

    def copy_files(self, train_dir, validation_dir):
        """
        ...
        _copy_files(src_paths=self.get_paths(validation=False),
                    dst_dir=train_dir,
                    class_numbers=self.class_numbers)

        print("- Copied training-set to:", train_dir)

        # Copy the files for the test-set.validation
        _copy_files(src_paths=self.get_paths(validation=True),
                    dst_dir=validation_dir,
                    class_numbers=self.class_numbers_validation)

        print("- Copied validation-set to:", validation_dir)
...
def load_cached(cache_path, in_dir):
    ...
    datasetvalidation = cache(cache_path=cache_path,
